        ...
        try {
            ...
            if (result.success) {
                ...
                if (this.connections.length > 0 && !this.activeConnection) {
                    ...
                    // Trigger portfolio loading
                    if (window.loadPlaidPortfolio) {
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-3">
                        ...
                        <div class="flex-1 min-w-0">
                            <h4 class="text-base font-semibold text-gray-900 truncate">${conn.institution_name}</h4>
                            <div class="flex flex-wrap gap-x-4 gap-y-1 mt-1">
                                <p class="text-sm text-gray-500">ID: ${conn.connection_id.substring(0, 12)}...</p>
                                <p class="text-sm text-gray-500">Created: ${new Date(conn.created_at).toLocaleDateString()}</p>
                    ...
                    <div class="flex items-center space-x-2 flex-shrink-0">
                        <button onclick="plaidConnectionsManager.selectConnection('${conn.connection_id}')" 
                                class="px-4 py-2 rounded-lg font-medium transition-all ${this.activeConnection === conn.connection_id ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}">
                        ...
                        <button onclick="plaidConnectionsManager.deleteConnection('${conn.connection_id}')" 
                                class="px-4 py-2 rounded-lg font-medium bg-red-100 text-red-700 hover:bg-red-200 transition-all">
                            Delete
                        </button>
                    </div>
                </div>
        ...
        container.innerHTML = `
            <div class="mb-4">
                <div class="flex items-center justify-between mb-3">
                    <h3 class="text-sm font-medium text-gray-900">Connected Accounts (${this.connections.length})</h3>
