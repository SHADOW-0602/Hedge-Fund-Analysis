...
class PerformanceAttributor:
    ...
    def factor_based_attribution(self, symbols: List[str], weights: Dict[str, float], 
        ...
        try:
            ...
            # Ensure weights dict exists - NO FALLBACK to equal weights
            if not weights:
                module_logger.warning("No weights provided for attribution")
                return self._empty_attribution_result()
            ...
            fetch_period = "max" if period.upper() == "ITD" else period
            data = self.data_client.get_price_data(symbols_to_fetch, fetch_period)
            ...
            if 'Asset Allocation' in attribution_types_list:
                if attribution_model == 'brinson':
                    ...
                    asset_allocation = self._calculate_asset_allocation_effect(returns, symbols, weights) # Fallback to standard
                else:
            ...
            if 'Security Selection' in attribution_types_list:
                if attribution_model == 'brinson':
                    ...
                    security_selection = self._calculate_security_selection_effect(returns, symbols, weights, benchmark_returns_series) # Fallback to standard
                else:
    ...
    def _calculate_market_timing_effect(self, portfolio_returns: pd.Series, benchmark_returns: pd.Series) -> float:
        ...
        try:
            ...
            timing_effect = timing_effect * 50
    ...
    def _calculate_currency_effect_enhanced(self, returns: pd.DataFrame, symbols: List[str], weights: Dict[str, float], currency: str, period: str) -> float:
        """Calculate currency effect based on foreign exposure"""
        try:
            ...
            if currency == 'USD':
                foreign_symbols = [s for s in symbols if any(x in s for x in intl_suffixes)]
                foreign_weight = sum(weights.get(s, 0) for s in foreign_symbols if s in returns.columns) / total_weight
            elif currency in ['CAD', 'EUR', 'GBP']:
                us_symbols = [s for s in symbols if not any(x in s for x in intl_suffixes)]
                foreign_weight = sum(weights.get(s, 0) for s in us_symbols if s in returns.columns) / total_weight
            else:
                foreign_symbols = [s for s in symbols if any(x in s for x in intl_suffixes)]
                foreign_weight = sum(weights.get(s, 0) for s in foreign_symbols if s in returns.columns) / total_weight

            if foreign_weight > 0:
                currency_effect = foreign_weight * portfolio_volatility * 0.15
            else:
                currency_effect = portfolio_volatility * 0.05
