...
def register_transaction_analysis_routes(app, data_client, smart_cache=None):
    ...
    def trade_performance():
        try:
            ...
            # Check cache with versioned key to avoid stale results
            print(f"[TRADE-PERFORMANCE] Options received: {options}")
            cache_key = cache_manager.generate_key('trade-performance-v2', data)
            cached_result = cache_manager.get(cache_key)
            if cached_result:
                return jsonify(cached_result)
            ...
            analyzer = AdvancedTransactionAnalyzer(data_client)
            performance_result = analyzer.trade_performance_analysis(transactions, options=options)
