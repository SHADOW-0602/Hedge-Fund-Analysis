...
def register_transaction_routes(app):
    ...
    def delete_transactions():
        try:
            ...
            return jsonify({'success': False, 'error': str(e)}), 500
    @app.route('/api/advanced-transaction-analysis', methods=['POST'])
    def advanced_transaction_analysis():
        try:
            import traceback
            from analytics.advanced_transaction_analysis import AdvancedTransactionAnalyzer
            from core.transactions import Transaction
            ...
            for tx_data in transactions_data:
                try:
                    ...
                    # Track positions for enterprise metrics
                    ...
                    if transaction.transaction_type == 'BUY':
                        ...
                        old_value = positions[symbol]['quantity'] * positions[symbol]['avg_cost']
                        new_value = abs(transaction.quantity) * transaction.price
                        total_quantity = positions[symbol]['quantity'] + abs(transaction.quantity)
                        ...
                        if symbol in positions and positions[symbol]['quantity'] > 0:
                            ...
                            avg_cost = positions[symbol]['avg_cost']
                            pnl = (transaction.price - avg_cost) * sell_quantity - transaction.fees
                            realized_pnl += pnl
                            positions[symbol]['quantity'] -= sell_quantity
            ...
            # Import data client from main app
            from clients.market_data_client import MarketDataClient
            data_client = MarketDataClient()
            
            analyzer = AdvancedTransactionAnalyzer(data_client)
            
            # Run all analyses for enterprise analytics
            enterprise_analytics = {
                'trade_performance': analyzer.trade_performance_analysis(transactions),
                'turnover_analysis': analyzer.turnover_analysis(transactions),
                'tax_loss_harvesting': analyzer.tax_loss_harvesting_analysis(transactions),
            ...
            trade_perf = enterprise_analytics['trade_performance']
            turnover = enterprise_analytics['turnover_analysis']
            drawdown = enterprise_analytics['drawdown_analysis']
            ...
            return jsonify({
                ...
                'summary': {
                    ...
                    'win_rate': trade_perf.get('win_rate', 0) * 100,
