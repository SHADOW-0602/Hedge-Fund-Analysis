...
def register_portfolio_routes(app, data_client, smart_cache=None):
    ...
    def analyze_risk():
        try:
            ...
            from utils.symbol_parser import get_underlying_symbol, parse_options_symbol
            symbols = []
            for p in portfolio_data:
                if isinstance(p, dict) and 'symbol' in p:
                    ...
                    if symbol and not symbol.startswith('CUR:') and not symbol.startswith('CASH'):
                        # For options, use underlying symbol for analysis
                        underlying = get_underlying_symbol(symbol)
                        if underlying and len(underlying) <= 10:
            ...
            for position in portfolio_data:
                ...
                if symbol and not symbol.startswith('CUR:') and not symbol.startswith('CASH'):
                    underlying = get_underlying_symbol(symbol)
                    if underlying and len(underlying) <= 10:
                        # Create new position with underlying symbol for analysis
                        new_position = position.copy()
                        new_position['symbol'] = underlying
                        filtered_portfolio_data.append(new_position)
            ...
            # Parse risk analysis options
            options = data.get('options', {})
            period = options.get('period', '1y')  # 1M, 3M, 6M, 1Y, 2Y, 3Y
            var_confidence = float(options.get('var_confidence', 95)) / 100  # 90%, 95%, 99%
            risk_model = options.get('risk_model', 'historical')  # Historical, Monte Carlo, Parametric
            benchmark = options.get('benchmark', 'SPY')  # S&P 500, NASDAQ, Russell 2000, Custom
            rolling_window = int(options.get('rolling_window', 252))  # 30d, 60d, 90d, 252d
            
            # Calculate fresh risk metrics with official Fed rate
            risk_free_rate = get_risk_free_rate()
            
            df = pd.DataFrame(filtered_portfolio_data)
            portfolio = Portfolio.from_dataframe(df)
            weights = portfolio.get_weights()
            metrics = risk_analyzer.analyze_portfolio_risk_fast(
    ...
    def monte_carlo():
        try:
            ...
            # Filter valid symbols with options parsing
            filtered_portfolio_data = []
            for position in portfolio_data:
                symbol = position.get('symbol', '').strip()
                if symbol and not symbol.startswith('CUR:') and not symbol.startswith('CASH'):
    ...
    def technical_analysis():
        try:
            ...
            return jsonify({'success': True, 'technical_analysis': results})
    ...
    def strategy_backtesting():
        try:
            ...
            return jsonify({'success': True, 'backtest': results})
    ...
    def statistical_analysis():
        try:
            ...
            return jsonify({'success': True, 'statistics': results})
    ...
    def pnl_attribution():
        try:
            ...
            if df.empty:
                return jsonify({'success': False, 'error': 'No transactions in selected period'}), 400
    ...
    def tax_analysis():
        try:
            ...
            if 'date' in df.columns:
                ...
                # Simplified tax year filtering - use all data for now
                if not df.empty:
                    # Use the most common year in the data
                    year_counts = df['date'].dt.year.value_counts()
                    if len(year_counts) > 0:
                        year = int(year_counts.index[0])  # Most common year
                    
                    # For testing, don't filter by year - use all data
    ...
    def cash_flow_analysis():
        try:
            ...
            df['cash_flow'] = df.apply(lambda row: 
                -float(row.get('quantity', 0)) * float(row.get('price', 0)) - float(row.get('fees', 0))
                if row.get('transaction_type', '').upper() == 'BUY'
                else float(row.get('quantity', 0)) * float(row.get('price', 0)) - float(row.get('fees', 0))
                if row.get('transaction_type', '').upper() == 'SELL'
                else float(row.get('quantity', 0)) * float(row.get('price', 0)), axis=1
            )
            ...
            df_grouped['inflows'] = df_grouped['cash_flow'].apply(lambda x: x if x > 0 else 0)
            df_grouped['outflows'] = df_grouped['cash_flow'].apply(lambda x: abs(x) if x < 0 else 0)
    ...
    def enhanced_backtesting_analysis():
        ...
        try:
            from analytics.enhanced_backtesting import EnhancedBacktesting
            data = request.get_json()
            ...
            enhanced_backtesting = EnhancedBacktesting()
            result = enhanced_backtesting.run_backtest(portfolio, options)
    ...
    def enhanced_monte_carlo_simulation():
        ...
        try:
            from analytics.enhanced_monte_carlo import EnhancedMonteCarloSimulator
            data = request.get_json()
            portfolio = data.get('portfolio', [])
            options = data.get('options', {})
            ...
            simulator = EnhancedMonteCarloSimulator(data_client)
        ...
        try:
            ...
            for bucket, bucket_data in bucket_groups:
                ...
                for _, trade in bucket_data.iterrows():
                    ...
                    # Calculate return based on market movement
                ...
                if returns:
                    ...
                    bucket_performance['win_rate'] = len([r for r in returns if r > 0]) / len(returns)
            ...
            best_time_bucket = max(timing_performance.items(), 
                                 key=lambda x: x[1]['avg_return']) if timing_performance else ('N/A', {'avg_return': 0})
            worst_time_bucket = min(timing_performance.items(), 
                                  key=lambda x: x[1]['avg_return']) if timing_performance else ('N/A', {'avg_return': 0})
            
            best_day = max(day_performance.items(), 
                         key=lambda x: x[1]['total_volume']) if day_performance else ('N/A', {'total_volume': 0})
    ...
    def drawdown_analysis():
        try:
            ...
            return jsonify({'success': True, 'drawdown_analysis': results})
