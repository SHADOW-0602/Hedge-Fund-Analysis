...
PLAID_ENVIRONMENT=sandbox  # or production
    ...
    def render_plaid_interface(self):
        """Main interface for Plaid connection"""
...
def integrate_plaid():
    ...
    with st.expander("🏦 Plaid Integration", expanded=False):
        multi_broker_connect.render_plaid_interface()
    
    # Return unified data for analysis
    return unified_client.get_all_holdings(multi_broker_connect.user_id)

if __name__ == "__main__":
    st.set_page_config(page_title="Plaid Connect", layout="wide")
    integrate_plaid()