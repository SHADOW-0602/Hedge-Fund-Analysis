...
try:
    import importlib
    pycaret_reg = importlib.import_module("pycaret.regression")
...
class MLPredictor:
    ...
    def train_return_prediction_model(self, symbols: List[str], 
        ...
        logger.info(f"Training ML models for {len(symbols)} symbols with {lookback_days} lookback days")
        ...
        for symbol in symbols:
            if symbol in price_data.columns:
                ...
                logger.info(f"Training models for {symbol} with {len(features)} data points")
                ...
                logger.info(f"Best model for {symbol}: {best_model_name} (R2: {best_model['r2']:.4f})")
...
class AutoMLEngine:
    ...
    def run_automl_experiment(self, symbols: List[str], target_days: int = 5) -> Dict:
        ...
        logger.info(f"Starting AutoML experiment for {len(symbols)} symbols")
        
        if not PYCARET_AVAILABLE:
            logger.warning("PyCaret not available for AutoML")
        ...
        for symbol in symbols:
            try:
                ...
                if symbol not in price_data.columns or len(price_data) < 500:
                    logger.warning(f"Insufficient data for AutoML on {symbol}")
                    continue
                
                logger.info(f"Running AutoML experiment for {symbol}")
...
class AdvancedMLPipeline:
    ...
    def run_comprehensive_analysis(self, symbols: List[str]) -> Dict:
        ...
        logger.info(f"Starting comprehensive ML analysis for {len(symbols)} symbols")
