...
class Config:
    ...
    PLAID_ENVIRONMENT: str = os.getenv('PLAID_ENVIRONMENT', 'production')
    PLAID_PRODUCTS: str = os.getenv('PLAID_PRODUCTS', 'investments,transactions,auth')
    PLAID_COUNTRY_CODES: str = os.getenv('PLAID_COUNTRY_CODES', 'US,CA')
    PLAID_REDIRECT_URI: str = os.getenv('PLAID_REDIRECT_URI', 'http://127.0.0.1:8080/plaid/callback')
    ...
    # ==================== DATA PROVIDERS ====================
    PROVIDER_PRIORITY: List[str] = os.getenv('PROVIDER_PRIORITY', 'polygon,finnhub,alpha_vantage,twelve_data,yfinance').split(',')
    ...
    def validate_config(cls) -> Dict[str, Any]:
        ...
        # Check Plaid
        if cls.PLAID_CLIENT_ID and cls.PLAID_SECRET:
        ...
        logger.info(f"Configuration validation complete - Services: {status['services']}")
        return status
    ...
    def setup_logging(cls):
        ...
        logger = logging.getLogger(__name__)
        logger.info(f"Configuration loaded - Environment: {cls.FLASK_ENV}, Debug: {cls.DEBUG}")
